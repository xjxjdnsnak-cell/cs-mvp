...
def calculate_player_round_impact(
    ...
    def _resolve_tick(target_time: float) -> PredictionTick | None:
        ...
        tick = find_exact_tick_via_full(
            round_context.full_ticks, round_context.ticks, target_time, tolerance=event_tolerance
        )
        if tick is None:
    ...
    def _resolve_ticks_before(target_time: float, max_seconds: float) -> list[PredictionTick]:
        return find_ticks_before_via_full(
            round_context.full_ticks, round_context.ticks, target_time, max_seconds
        )

    for kill in kills:
        before_tick = _resolve_tick(kill.tick - event_window_offset)
        after_tick = _resolve_tick(kill.tick + event_window_offset)
        risk_window = _resolve_ticks_before(kill.tick, max_seconds=10.0)
    ...
    for death in deaths:
        ...
        risk_window = _resolve_ticks_before(death.tick, max_seconds=10.0)
...
def calculate_player_match_impact(
    ...
    rule_quality_score = rule_quality_score_raw[1]
    ...
    return PlayerMatchImpact(
        ...
        model_impact_score_raw=model_impact_score_raw[0],
        model_impact_score_clipped=model_impact_score,
        rule_quality_score_raw=rule_quality_score_raw[0],
...
def calculate_model_impact_score_raw(
    ...
    """Calculate the model-based impact score. Returns (raw, clipped)."""
    ...
    return model_score, max(-50, min(50, model_score))
...
def calculate_rule_quality_score_raw(
    ...
    """Calculate the rule-based quality score. Returns (raw, clipped)."""
    ...
    return rule_score, max(-50, min(50, rule_score))
