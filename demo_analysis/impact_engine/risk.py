...
def assess_death_risk(
    ...
    if player_is_on_team1:
        player_alive_advantage = team1_alive - team2_alive
        own_alive = team1_alive
        enemy_alive = team2_alive
    else:
        player_alive_advantage = team2_alive - team1_alive
        own_alive = team2_alive
    ...
    # 用 own_alive 直接表示本方存活人数，无需 +1
    players_dead = 10 - (team1_alive + team2_alive)
    enemies_killed = (5 - enemy_alive)

    team_has_advantage = own_alive >= enemy_alive
    ...
    was_entry = False
    if player_alive_advantage >= 1 and distant_from_team and not bomb_was_planted:
    ...
    if distant_from_team and team_has_advantage and not bomb_was_planted:
        ...
        reasons.append(f"人数优势({own_alive}v{enemy_alive})时远离队友({nearest_teammate_dist:.0f}单位)单摸")
    ...
    if player_alive_advantage <= -1:
        forced_score += 2
        reasons.append("人数劣势被迫接敌")

    if bomb_was_planted and own_alive <= 2:
        forced_score += 2
    ...
    if player_alive_advantage >= 0 and not distant_from_team:
        forced_score += 1
    ...
    if self_created_score >= 3:
        ...
        return RiskAssessment(
            ...
            team_alive_advantage=player_alive_advantage,
        )
